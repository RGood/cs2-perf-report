...
RULES_NEG = {
    ...
    'slow_to_damage': ("Slow to damage", "More than 0.7 s passed between an enemy first coming into your view and your first damage on them, and you died to them.", "Decide before the peek. If the crosshair is placed, the first shot goes out on sight."),
    ...
    'ran_into_contact': ("Ran into contact", "You were running (over 200 u/s) while not yet visible, with an enemy within 20 m who was not yet spotted, and lost the fight that followed. Running makes noise the enemy hears before you see them.", "Walk the last 20 m into any position an enemy could hold. Sound is information you give away."),
...
RULES_POS = {
    ...
    'post_plant_hold': ("Post-plant hold", "After the plant you survived 20 s or more and got a kill, or the bomb exploded with you alive.", "Keep the post-plant positions that make the retake cost players."),
    ...
    'defuse_fake': ("Defuse fake worked", "You started a defuse and stopped without finishing, and an enemy died to your team within 3 s.", "Keep faking when the enemy is holding the bomb from a spot a teammate can punish."),
...
BASE_POS = {'opener_traded': 25, 'pre_aimed': 8, 'prefired': 12, 'first_bullet_hit': 6, 'won_after_hit_first': 15, 'counter_strafed': 6, 'wallbang_kill': 15, 'kill_through_smoke': 15,
            'noscope_kill': 10, 'unseen_kill': 12, 'kill_with_cover': 12, 'info_peek_survived': 10, 'caught_rotation': 35, 'post_plant_hold': 38, 'kill_down_a_man': 40, 'weapon_drop': 10,
            ...
            'picked_rifle_on_eco': 10, 'plant_under_pressure': 35, 'fast_plant': 10, 'defused_under_fire': 55, 'sneaky_defuse': 55, 'defuse_fake': 40, 'pistol_switch_won': 15}
...
def negatives(D, me):
            ...
            if killer in foes:
                ...
                if fs is not None:
                    ...
                    if len(hk) and (int(hk.iloc[0]['tick']) - fs) / TICK > 0.7:
                        out.append(c.base(rn, side, t, place, pos, kind='slow_to_damage', facts=base_facts + f" {d.attacker_name} came into your view at {c.rt(fs, rn)} s; your first damage on them came {(int(hk.iloc[0]['tick']) - fs) / TICK:.1f} s later.", **kw))
    ...
    return [o for o in out if o.get('pos') is not None]
    ...
    for rn in sorted(fz):
        ...
        if len(pl) and pl.iloc[0]['user_X'] == pl.iloc[0]['user_X']:
            ...
            if side == 'T':
                ...
                if alive_at and (len(k_after) or (boom and (not len(rdm) or int(rdm.iloc[0]['tick']) > int(ex[ex['total_rounds_played'] == rn].iloc[0]['tick'])))):
                    mr = c.row(t_pl + 20 * TICK, me)
                    if mr is not None: out.append(c.base(rn, side, t_pl + 20 * TICK, str(mr['last_place_name']), (float(mr.X), float(mr.Y)), kind='post_plant_hold', facts=f"Round {rn+1}, T, plant at {c.rt(t_pl, rn)} s. You survived 20 s after it" + (f" and got {len(k_after)} kill(s)" if len(k_after) else "") + (" and the bomb exploded." if boom else "."), extra_pos=bpos, extra_label='bomb'))
